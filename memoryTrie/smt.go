...
func NewSMT() *SMT {
	s := &SMT{
		...
		TrieHeight: len(Hasher([]byte("height"))) * 8,
	}
	s.db = &cacheDB{
...
func (s *SMT) update(root []byte, keys, values, batch [][]byte, iBatch, height int, ch chan<- (mresult)) {
...
func (s *SMT) updateRight(lnode, rnode, root []byte, keys, values, batch [][]byte, iBatch, height int, ch chan<- (mresult)) {
...
func (s *SMT) updateLeft(lnode, rnode, root []byte, keys, values, batch [][]byte, iBatch, height int, ch chan<- (mresult)) {
...
func (s *SMT) updateParallel(lnode, rnode, root []byte, lkeys, rkeys, lvalues, rvalues, batch [][]byte, iBatch, height int, ch chan<- (mresult)) {
...
func (s *SMT) maybeMoveUpShortcut(left, right, root []byte, batch [][]byte, iBatch, height int, ch chan<- (mresult)) bool {
...
func (s *SMT) moveUpShortcut(shortcut, root []byte, batch [][]byte, iBatch, iShortcut, height int, ch chan<- (mresult)) {
...
func (s *SMT) loadBatch(root []byte) ([][]byte, error) {
	...
	s.loadDbMux.Lock()
	dbval, err := s.db.getNodeDB(root[:HashLength])
	s.loadDbMux.Unlock()
	if err != nil {
		return nil, err
	}
...
func (s *SMT) parseBatch(val []byte) ([][]byte, error) {
	...
	if bitIsSet(val, 31) {
		if len(val) < 70 {
			return nil, errors.New("length of input value should be seventy-one for a shortcut")
		}
		batch[0] = []byte{1}
		batch[1] = val[4 : 4+33]
	...
	if subtreeHeight > 0 {
		...
		if len(lres) == 0 && len(rres) == 0 {
			...
			if batch[idx][32] == 1 && idx < 16 {
				if useUniformLeafHeight == true {
					side := bitIsSet(batch[lidx], 255-(height+subtreeHeight-1))
					val := s.shortcutToSubTreeRoot(batch[lidx][:HashLength], batch[ridx][:HashLength], height+subtreeHeight-1)
					if side == false {
						res = s.hash(val, s.defaultHashes[height+subtreeHeight-1])
					} else {
						res = s.hash(s.defaultHashes[height+subtreeHeight-1], val)
					}
				} else {
					res = s.hash(batch[lidx][:HashLength], batch[ridx][:HashLength], []byte{byte(height + subtreeHeight)})
				}

			} else if len(batch[ridx]) > 0 && len(batch[lidx]) > 0 {
				res = s.hash(batch[lidx][:HashLength], batch[ridx][:HashLength])
			} else if len(batch[ridx]) > 0 {
				if useUniformLeafHeight {
					res = s.hash(s.defaultHashes[height+subtreeHeight-1], batch[ridx][:HashLength])
				} else {
					res = s.hash(DefaultLeaf, batch[ridx][:HashLength])
				}
			} else if len(batch[lidx]) > 0 {
				if useUniformLeafHeight {
					res = s.hash(batch[lidx][:HashLength], s.defaultHashes[height+subtreeHeight-1])
				} else {
					res = s.hash(batch[lidx][:HashLength], DefaultLeaf)
				}
			}
			...
			if useUniformLeafHeight {
				res = s.hash(lres, s.defaultHashes[height+subtreeHeight-1])
			} else {
				res = s.hash(lres, DefaultLeaf)
			}
		} else if len(rres) > 0 {
			if useUniformLeafHeight {
				res = s.hash(s.defaultHashes[height+subtreeHeight-1], rres)
			} else {
		...
		if idx == 0 {
			...
			equiv = bytes.Equal(res, batch[idx][:HashLength])
		}
		if equiv == false {
			// a few things for debugging if something does not pass through correctly
			fmt.Printf("%x\n", res)
			fmt.Println(batch[idx][:HashLength])
			fmt.Println(len(lres), len(rres))
			fmt.Println(lidx, ridx, idx, batch[idx][32])
		}
	}
