...
// GetMaxTxVectorLength returns the max length for Tx.Vin and Tx.Vout
func (s *Storage) GetMaxTxVectorLength() int {
	select {
	case <-s.startChan:
	}
	s.RLock()
	defer s.RUnlock()
	return s.rawStorage.GetMaxTxVectorLength()
}
