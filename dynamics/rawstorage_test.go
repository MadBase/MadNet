...
func TestRawStorageStandardParameters(t *testing.T) {
	...
	retMaxTxVectorLength := rs.GetMaxTxVectorLength()
	if retMaxTxVectorLength != maxTxVectorLength {
...
func TestRawStorageMaxTxVectorLength(t *testing.T) {
	rs1 := &RawStorage{}
	retMaxTxLength0 := rs1.GetMaxTxVectorLength()
	if retMaxTxLength0 != 0 {
		t.Fatal("maxTxVectorLength should be 0")
	}

	rs2 := &RawStorage{}
	err := rs2.SetMaxTxVectorLength(0)
	...
	rs3 := &RawStorage{}
	maxLength := int32(1)
	err = rs3.SetMaxTxVectorLength(maxLength)
	if err != nil {
		t.Fatal(err)
	}
	retMaxTxLength1 := rs3.GetMaxTxVectorLength()
	if retMaxTxLength1 != int(maxLength) {
		t.Fatal("maxTxVectorLengths do not agree")
	}
}
...
func TestRawStorageUpdateValueMaxTxVectorLength(t *testing.T) {
	rs := &RawStorage{}

	retMaxLength := rs.GetMaxTxVectorLength()
	if retMaxLength != 0 {
		t.Fatal("Incorrect MaxTxVectorLength (1)")
	}

	field := "maxTxVectorLength"
	valueBad1 := ""
	epoch := uint32(1)
	update, err := NewUpdate(field, valueBad1, epoch)
	if err != nil {
	...
	maxLengthTrue := int(valueTrue64)
	retMaxLength = rs.GetMaxTxVectorLength()
	if retMaxLength != maxLengthTrue {
		t.Fatal("Incorrect MaxTxVectorLength (2)")
	}
}
