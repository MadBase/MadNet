...
func TestStorageInitialized(t *testing.T) {
	...
	maxTxVectorLengthReturned := s.GetMaxTxVectorLength()
	if maxTxVectorLengthReturned != maxTxVectorLength {
		t.Fatal("Incorrect maxTxVectorLength")
	}
...
func TestStorageGetMaxTxVectorLength(t *testing.T) {
	s := initializeStorageWithFirstNode()
	maxLength := s.GetMaxTxVectorLength()
	if maxLength != maxTxVectorLength {
		t.Fatal("incorrect maxTxVectorLength")
	}
	...
	valueTrue, err := stringToInt32(value)
	if err != nil {
		t.Fatal(err)
	}
