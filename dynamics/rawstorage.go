...
type RawStorage struct {
	...
	MaxTxVectorLength              int32         `json:"maxTxVectorLength,omitempty"`
...
func (rs *RawStorage) UpdateValue(update Updater) error {
	...
	case MsgTimeoutType:
		...
		rs.SetMsgTimeout(v)
	case MaxTxVectorLengthType:
		// int32
		v, err := stringToInt32(value)
		if err != nil {
			return err
		}
		err = rs.SetMaxTxVectorLength(v)
		if err != nil {
			return err
		}
	case MinTxFeeType:
...
func (rs *RawStorage) standardParameters() {
	...
	rs.MaxTxVectorLength = int32(maxTxVectorLength)
}
...
// GetMaxTxVectorLength returns the max value of Tx.Vin and Tx.Vout
func (rs *RawStorage) GetMaxTxVectorLength() int {
	return int(rs.MaxTxVectorLength)
}

// SetMaxTxVectorLength sets the max value of Tx.Vin and Tx.Vout
func (rs *RawStorage) SetMaxTxVectorLength(value int32) error {
	if value <= 0 {
		return ErrInvalidValue
	}
	rs.MaxTxVectorLength = value
	return nil
}
