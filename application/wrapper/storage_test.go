...
func TestStorageGetAtomicSwapFeeFailsWhenNotInitialised(t *testing.T) {
	s := make([]*Storage, 1)
	fee, err := s[0].GetAtomicSwapFee()
	assert.Error(t, err)
	assert.Nil(t, fee)
}
...
func TestStorageWithoutStorageGetterGetAtomicSwapFeeFails(t *testing.T) {
	s := &Storage{}
...
func TestStorageInitialisedReturnsExpectedMaxBytes(t *testing.T) {
	...
	msg.SetMaxBytes(expectedMaxBytes)
...
func TestStorageInitialisedReturnsExpectedAtomicSwapFee(t *testing.T) {
	msg := MakeMockStorageGetter()
	s := NewStorage(msg)
